import { Logo } from "./logo";

const GITHUB_URL = "https://github.com/onfable/onfable";
const X_URL = "https://x.com/onfable";

export function Nav() {
  return (
    <header className="sticky top-0 z-50 border-b border-edge bg-black/60 backdrop-blur-md">
      <nav className="mx-auto flex h-16 max-w-6xl items-center justify-between px-6">
        <a href="#" aria-label="onfable home">
          <Logo />
        </a>
        <div className="flex items-center gap-4 text-sm text-muted">
          <a href="#demo" className="hidden px-1 transition-colors hover:text-white sm:block">
            Demo
          </a>
          <a href="#install" className="hidden px-1 transition-colors hover:text-white sm:block">
            Install
          </a>
          <a href="#features" className="hidden px-1 transition-colors hover:text-white sm:block">
            Features
          </a>
          <a href="#roadmap" className="hidden px-1 transition-colors hover:text-white sm:block">
            Roadmap
          </a>
          <a href="#token" className="hidden px-1 transition-colors hover:text-white sm:block">
            $ONFABLE
          </a>
          <a href="#faq" className="hidden px-1 transition-colors hover:text-white sm:block">
            FAQ
          </a>
          <span aria-hidden="true" className="hidden h-4 w-px bg-edge sm:block" />
          <div className="flex items-center gap-1.5">
            <a
              href={X_URL}
              target="_blank"
              rel="noopener noreferrer"
              aria-label="onfable on X"
              className="rounded-md p-2 transition-colors hover:bg-card hover:text-white"
            >
              <svg viewBox="0 0 24 24" className="h-4 w-4 fill-current">
                <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
              </svg>
            </a>
            <a
              href={GITHUB_URL}
              target="_blank"
              rel="noopener noreferrer"
              aria-label="onfable on GitHub"
              className="rounded-md p-2 transition-colors hover:bg-card hover:text-white"
            >
              <svg viewBox="0 0 16 16" className="h-4 w-4 fill-current">
                <path d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27s1.36.09 2 .27c1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.01 8.01 0 0 0 16 8c0-4.42-3.58-8-8-8z" />
              </svg>
            </a>
          </div>
        </div>
      </nav>
    </header>
  );
}
